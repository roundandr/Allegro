...
MMA, COMMIT, WAIT, STORE = range(1,5)
...
async def issue(dut, opcode, tag, *, a=0, b=0, dst=0, slot=0,
                accumulate=0, token=0, timeout=20000):
...
async def full_tile_overwrite_accumulate_slots_sync(dut):
    ...
    commit = await issue(dut, COMMIT, 0x20)
    await issue(dut, WAIT, 0x21, token=commit[3])

    assert int(dut.perf_issued_o.value) >= 10
    assert int(dut.perf_completed_o.value) >= 9
