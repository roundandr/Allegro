// ============================================================================
// File Name   : f16tf32_s0_s2_frontend.sv
// Author      : LIU YUXUAN
// Date        : 2026-06-03
// Description : F16TF32 S0-S2 frontend for the shared F16TF32/F4F6F8 dot-product
//               tail.  The output payload is a 33-term signed fixed-point
//               vector ready for the shared accumulation tree.
// ============================================================================

module f16tf32_s0_s2_frontend #(
    ...
    always_comb begin
        ...
        reserved_dtype_tmp    = (f16tf32_dtype_i == DOT_F16TF32_DTYPE_RSVD);
        ...
        for (idx0 = 0; idx0 < DOT_F16TF32_NUM_ELEMS; idx0 = idx0 + 1) begin
            ...
            if (!reserved_dtype_tmp &&
                (f16tf32_dtype_i == DOT_F16TF32_DTYPE_TF32)) begin
                ...
                a_dec_tmp = decode_fp16_bf16(f16tf32_dtype_i == DOT_F16TF32_DTYPE_BF16,
                                             a_vec_i[idx0*DOT_F16TF32_FP16_W +: DOT_F16TF32_FP16_W]);
                b_dec_tmp = decode_fp16_bf16(f16tf32_dtype_i == DOT_F16TF32_DTYPE_BF16,
    ...
    logic signed [DOT_F16TF32_EXP_W-1:0] base_exp_s2_tmp;
    always_comb begin
        s2_d = '0;
        s2_d.meta = s1_q.meta;
        s2_term_flat_tmp = '0;
        ...
        s2_d.special_vld = (s1_q.special_code != DOT_SPECIAL_NONE);
        s2_d.special_result = dot_pack_special_fp32(s1_q.special_code);
        base_exp_s2_tmp = s1_q.emax - DOT_F16TF32_ALIGN_FRAC_BITS_EXP;
        s2_d.base_exp = $signed({base_exp_s2_tmp[DOT_F16TF32_EXP_W-1], base_exp_s2_tmp});

        for (int idx2 = 0; idx2 < DOT_F16TF32_NUM_ELEMS; idx2 = idx2 + 1) begin
            s2_term_flat_tmp[(idx2+1)*SHARED_SUM_W +: SHARED_SUM_W] =
                $signed(s2_prod_aligned_flat[idx2*DOT_F16TF32_ALIGN_TERM_W +: DOT_F16TF32_ALIGN_TERM_W]);
        end

        s2_term_flat_tmp[0*SHARED_SUM_W +: SHARED_SUM_W] = $signed(s2_c_aligned);
