...
module blackwell_tensor_subsystem #(
    ...
    input  logic [3:0]    cmd_barrier_id_i,
    input  logic          cmd_barrier_phase_i,
    ...
    output logic          tma_req_vld_o,
    input  logic          tma_req_rdy_i,
    output logic [15:0]   tma_req_tag_o,
    output logic [31:0]   tma_req_src_addr_o,
    output logic [31:0]   tma_req_dst_addr_o,
    output logic [15:0]   tma_req_bytes_o,
    output logic [3:0]    tma_req_barrier_id_o,
    output logic          tma_req_phase_o,
    input  logic          tma_done_vld_i,
    output logic          tma_done_rdy_o,
    input  logic [15:0]   tma_done_tag_i,
    input  logic [3:0]    tma_done_barrier_id_i,
    input  logic          tma_done_phase_i,
    input  logic [1:0]    tma_done_status_i,
    ...
    localparam logic [3:0] ST_WAIT_BARRIER    = 4'd14;
    ...
    logic tile_slot_q;
    logic accumulate_q;
    logic [3:0] barrier_id_q;
    logic barrier_phase_q;
    ...
    logic [7:0] tma_outstanding_q;
    logic [7:0] async_error_q;
    logic active_work_q;
    logic [15:0] tma_tag_q [0:15];
    logic tma_busy_q [0:15];
    logic barrier_phase_table_q [0:15];
    ...
    logic tma_issue_fire;
    logic tma_done_fire;
    logic tma_done_counted;
    ...
    always_comb begin
        ...
        if ((state_q == ST_IDLE) && completion_slot_rdy &&
            ...
            if (cmd_opcode_i == BW_CMD_TMA_REQ) begin
                cmd_rdy_comb = tma_req_rdy_i && (tma_outstanding_q != 8'hff) &&
                                !tma_busy_q[cmd_barrier_id_i];
            end else begin
                cmd_rdy_comb = 1'b1;
            end
        end
    ...
    assign tma_req_vld_o = (state_q == ST_IDLE) && completion_slot_rdy &&
                           cmd_vld_i && (cmd_opcode_i == BW_CMD_TMA_REQ) &&
                           (tma_outstanding_q != 8'hff) &&
                           !tma_busy_q[cmd_barrier_id_i];
    assign tma_req_tag_o = cmd_tag_i;
    assign tma_req_src_addr_o = cmd_a_base_i;
    assign tma_req_dst_addr_o = cmd_dst_base_i;
    assign tma_req_bytes_o = 16'd256;
    assign tma_req_barrier_id_o = cmd_barrier_id_i;
    assign tma_req_phase_o = cmd_barrier_phase_i;
    assign tma_issue_fire = tma_req_vld_o && tma_req_rdy_i;

    assign tma_done_rdy_o = 1'b1;
    assign tma_done_fire = tma_done_vld_i && tma_done_rdy_o;
    assign tma_done_counted = tma_done_fire &&
                              tma_busy_q[tma_done_barrier_id_i];
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state_q <= ST_IDLE;
            active_tag_q <= 16'd0;
            active_opcode_q <= BW_CMD_TMA_REQ;
            a_base_q <= 32'd0;
            b_base_q <= 32'd0;
            dst_base_q <= 32'd0;
            tile_slot_q <= 1'b0;
            ...
            completion_opcode_q <= BW_CMD_TMA_REQ;
            completion_status_q <= BW_STATUS_OK;
            completion_token_q <= 16'd0;
            issued_watermark_q <= 16'd0;
            retired_watermark_q <= 16'd0;
            tma_outstanding_q <= 8'd0;
            async_error_q <= BW_STATUS_OK;
            active_work_q <= 1'b0;
            for (barrier_idx = 0; barrier_idx < 16; barrier_idx = barrier_idx + 1) begin
                tma_tag_q[barrier_idx] <= 16'd0;
                tma_busy_q[barrier_idx] <= 1'b0;
                barrier_phase_table_q[barrier_idx] <= 1'b0;
            end
        end else begin
            ...
            case ({tma_issue_fire, tma_done_counted})
                2'b10: tma_outstanding_q <= tma_outstanding_q + 8'd1;
                2'b01: begin
                    if (tma_outstanding_q != 8'd0) begin
                        tma_outstanding_q <= tma_outstanding_q - 8'd1;
                    end
                end
                default: tma_outstanding_q <= tma_outstanding_q;
            endcase

            if (tma_issue_fire) begin
                tma_tag_q[cmd_barrier_id_i] <= cmd_tag_i;
                tma_busy_q[cmd_barrier_id_i] <= 1'b1;
            end
            if (tma_done_fire) begin
                if (tma_busy_q[tma_done_barrier_id_i]) begin
                    tma_busy_q[tma_done_barrier_id_i] <= 1'b0;
                end
                if (tma_busy_q[tma_done_barrier_id_i] &&
                    (tma_done_status_i == 2'd0) &&
                    (tma_done_tag_i == tma_tag_q[tma_done_barrier_id_i])) begin
                    barrier_phase_table_q[tma_done_barrier_id_i] <= tma_done_phase_i;
                end else begin
                    async_error_q <= BW_STATUS_TMA;
                end
                if (!tma_issue_fire && tma_done_counted &&
                    (tma_outstanding_q == 8'd1) && !active_work_q) begin
            ...
            case (state_q)
                ST_IDLE: begin
                    if (cmd_fire) begin
                        ...
                        barrier_id_q <= cmd_barrier_id_i;
                        barrier_phase_q <= cmd_barrier_phase_i;
                        ...
                        if (alignment_error) begin
                            ...
                            case (cmd_opcode_i)
                                BW_CMD_TMA_REQ: begin
                                    issued_watermark_q <= issued_watermark_q + 16'd1;
                                    completion_vld_q <= 1'b1;
                                    completion_tag_q <= cmd_tag_i;
                                    completion_opcode_q <= cmd_opcode_i;
                                    completion_status_q <= BW_STATUS_OK;
                                    completion_token_q <= issued_watermark_q + 16'd1;
                                end
                                BW_CMD_MMA: begin
                                    issued_watermark_q <= issued_watermark_q + 16'd1;
                                    active_work_q <= 1'b1;
                                ...
                                BW_CMD_WAIT: begin
                                    if (retired_watermark_q >= cmd_wait_token_i) begin
                                        ...
                                        completion_tag_q <= cmd_tag_i;
                                        completion_opcode_q <= cmd_opcode_i;
                                        completion_status_q <= async_error_q;
                                ...
                                BW_CMD_MBARRIER_WAIT: begin
                                    if (barrier_phase_table_q[cmd_barrier_id_i] == cmd_barrier_phase_i) begin
                                        completion_vld_q <= 1'b1;
                                        completion_tag_q <= cmd_tag_i;
                                        completion_opcode_q <= cmd_opcode_i;
                                        completion_status_q <= BW_STATUS_OK;
                ...
                ST_MMA_B_RSP: begin
                    if (smem_b_rsp_vld_eff &&
                        ...
                        if ((smem_b_rsp_status_eff != 2'd0) ||
                            ...
                            completion_token_q <= issued_watermark_q;
                            active_work_q <= 1'b0;
                            if (tma_outstanding_q == 8'd0) retired_watermark_q <= issued_watermark_q;
                            state_q <= ST_IDLE;
                        end else begin
                ...
                ST_WAIT_BARRIER: begin
                    if ((barrier_phase_table_q[barrier_id_q] == barrier_phase_q) && completion_slot_rdy) begin
                        completion_vld_q <= 1'b1;
                        completion_tag_q <= active_tag_q;
                        completion_opcode_q <= active_opcode_q;
                        completion_status_q <= BW_STATUS_OK;
            ...
            // Eventual retirement closes the simultaneous final-TMA/final-MMA
            // corner: after both active flags clear, the full accepted
            // watermark becomes observable to WAIT on the following cycle.
            if (!active_work_q && (tma_outstanding_q == 8'd0)) begin
                retired_watermark_q <= issued_watermark_q;
            end
        end
    end
