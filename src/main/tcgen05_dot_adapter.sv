...
module tcgen05_dot_adapter (
    ...
    logic [3:0] active_d_type_q;
    logic       active_is_fp4_q;
        ...
        if (err_vld_q) begin
            ...
            if ((active_core_q == CORE_F16TF32) && !active_is_fp4_q) begin
                ...
                d_o = (active_d_type_q == TCGEN05_TYPE_F16) ? {16'h0000, fp32_to_fp16_rne(f4f6f8_d)} :
                                                              f4f6f8_d;
            end else if (active_core_q == CORE_INT8) begin
                out_vld_o = int8_out_vld;
                d_o = int8_d;
            end else if (active_is_fp4_q) begin
                out_vld_o = fp4_out_vld;
                d_o = fp4_d;
            end
        end
    ...
    assign core_out_fire = out_vld_o && out_rdy_i && !err_vld_q;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            busy_q         <= 1'b0;
            err_vld_q      <= 1'b0;
            err_status_q   <= TCGEN05_STATUS_OK;
            active_core_q  <= CORE_NONE;
            active_d_type_q <= TCGEN05_TYPE_F32;
            active_is_fp4_q <= 1'b0;
        end else begin
            if (err_vld_q && out_rdy_i) begin
                err_vld_q <= 1'b0;
            end
            ...
            if (issue_fire) begin
                if (err_path) begin
                    err_vld_q <= 1'b1;
                    err_status_q <= invalid_sparse_meta ? TCGEN05_STATUS_INVALID_SPARSE_META :
                                                          TCGEN05_STATUS_UNSUPPORTED;
                end else begin
                    busy_q          <= 1'b1;
                    active_core_q   <= selected_core;
                    active_d_type_q <= d_type_i;
                    active_is_fp4_q <= selected_is_fp4;
                    err_status_q    <= TCGEN05_STATUS_OK;
                end
            end
        end
