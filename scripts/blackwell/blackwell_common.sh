...
require_blackwell_environment() {
  ...
  for filelist in filelists/blackwell_async_filelist.f \
                  filelists/blackwell_tensor_filelist.f \
                  filelists/tma_mbarrier_filelist.f \
                  filelists/blackwell_subsystem_filelist.f; do
    if [[ ! -f "${filelist}" ]]; then
